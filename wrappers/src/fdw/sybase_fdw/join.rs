...
struct JoinPlanData {
    ...
    sql: String,
    /// Foreign server both relations live on
    server_oid: Oid,
...
/// Execution state for a join scan, rebuilt from the plan's fdw_private on
/// every BeginForeignScan (a cached generic plan runs many times).
struct JoinScanState {
    /// The complete join SQL to execute remotely
    sql: String,
    /// ODBC connection string, resolved at execution time so the password
    /// never lands in the plan tree
    conn_str: String,
...
unsafe extern "C-unwind" fn sybase_get_foreign_join_paths(
    ...
    unsafe {
        ...
        // Cost estimation: the cost should reflect that a remote join
        // query has ODBC overhead (startup) plus per-row transfer cost.
        // Using cost proportional to rows (same as base foreign scans)
        // allows the planner to fairly compare join pushdown vs
        // parameterized nested loop with individual table scans.
        let rows = (*outerrel).rows * (*innerrel).rows * 0.01;
        let rows = if rows < 1.0 { 1.0 } else { rows };
        let startup_cost = 10.0;
        let total_cost = startup_cost + rows;
...
pub(super) unsafe fn get_server_conn_str(server_oid: Oid) -> Option<String> {
    unsafe {
...
// an aggregate-pushdown upper-rel plan (also scanrelid=0), whose list starts
// with the foreign table OID.
const JOIN_PRIVATE_MARKER: i64 = 0x5359_4253_4A4F_494E; // 'SYBSJOIN'

/// Serializes the join plan as a list of Const nodes:
/// `[marker, server_oid, sql, type_oid...]`. Everything is copied by value so
/// the list survives the plan-cache `copyObject` and every EXECUTE of a
/// cached generic plan rebuilds its own state from it.
unsafe fn serialize_join_plan(plan_data: &JoinPlanData) -> *mut pg_sys::List {
    unsafe {
        let by_val = |type_oid: Oid, len: c_int, datum: Datum| {
            pg_sys::makeConst(type_oid, -1, pg_sys::InvalidOid, len, datum, false, true)
        };
        let mut consts = vec![
            by_val(
                pg_sys::INT8OID,
            ...
            by_val(
                pg_sys::OIDOID,
                4,
                plan_data.server_oid.into_datum().unwrap(),
            ),
            pg_sys::makeConst(
                pg_sys::TEXTOID,
                -1,
                ...
                -1,
                plan_data.sql.as_str().into_datum().unwrap(),
                false,
                false,
            ),
        ];
        for tc in &plan_data.target_columns {
            consts.push(by_val(pg_sys::OIDOID, 4, tc.type_oid.into_datum().unwrap()));
        }
...
/// Returns the Const values of `list` when it is a join private list, or
/// `None` so the caller falls back to the framework's scan/upper-rel
/// callbacks instead of misinterpreting their state.
unsafe fn join_plan_consts(list: *mut pg_sys::List) -> Option<Vec<pg_sys::Const>> {
    unsafe {
        pgrx::memcx::current_context(|mcx| {
            let list = List::<*mut c_void>::downcast_ptr_in_memcx(list, mcx)?;
            let consts: Vec<pg_sys::Const> = list
                .iter()
                .map(|node| *(*node as *mut pg_sys::Const))
                .collect();
            let marker = consts.first()?;
            (marker.consttype == pg_sys::INT8OID
                && i64::from_datum(marker.constvalue, marker.constisnull)
                    == Some(JOIN_PRIVATE_MARKER)
                && consts.len() >= 3)
                .then_some(consts)
        })
    }
...
unsafe fn is_join_scan(plan: *mut pg_sys::ForeignScan) -> bool {
    unsafe { (*plan).scan.scanrelid == 0 && join_plan_consts((*plan).fdw_private).is_some() }
}
...
unsafe extern "C-unwind" fn sybase_get_foreign_plan(
    ...
    unsafe {
        ...
        let fdw_private = serialize_join_plan(plan_data);
...
unsafe extern "C-unwind" fn sybase_begin_foreign_scan(
    ...
    unsafe {
        ...
        // the marker in fdw_private tells them apart.
        let consts = if (*plan).scan.scanrelid == 0 {
            join_plan_consts((*plan).fdw_private)
        } else {
            None
        };
        ...
        let server_oid = Oid::from_datum(consts[1].constvalue, false).unwrap();
        let mut state = JoinScanState {
            sql: String::from_datum(consts[2].constvalue, false).unwrap(),
            conn_str: String::new(),
            type_oids: consts[3..]
                .iter()
        ...
        // Dropped in `sybase_end_foreign_scan`.
...
unsafe extern "C-unwind" fn sybase_iterate_foreign_scan(
    ...
    unsafe {
        ...
        if !is_join_scan((*node).ss.ps.plan as _) {
            let orig = ORIGINAL_CALLBACKS.get().unwrap();
        ...
        for (i, &type_oid) in state.type_oids.iter().enumerate() {
            let val = row.get(i).and_then(|v| v.as_deref());
            let cell = value_to_cell(val, type_oid).unwrap_or(None);
...
unsafe extern "C-unwind" fn sybase_explain_foreign_scan(
    ...
    unsafe {
        ...
        let value =
            PgMemoryContexts::CurrentMemoryContext.pstrdup(&format!("Remote SQL: {}", state.sql));
