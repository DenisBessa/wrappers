...
use super::join::get_server_conn_str;
...
impl ForeignDataWrapper<SybaseFdwError> for SybaseFdw {
    ...
    fn get_rel_size(
        ...
        if let Ok(cache) = ROW_COUNT_CACHE.lock()
            && let Some(&count) = cache.get(&table)
        {
            let width = if columns.is_empty() {
                100
            } else {
        ...
        let rows = if !table.is_empty() {
            ...
            if parts.len() == 2 {
                ...
                // Planning runs before any FDW instance exists, so resolve the
                // connection string from the server the framework recorded.
                let conn_str = options
                    .get("wrappers.fserver_oid")
                    .and_then(|oid| oid.parse::<u32>().ok())
                    .and_then(|oid| unsafe { get_server_conn_str(Oid::from(oid)) })
                    .unwrap_or_default();
                match connect_unlocked(&conn_str) {
                    Ok(conn) => {
                        let sql = format!(
        ...
        if !self.query_executed {
            ...
            for qual in &mut self.stored_quals {
                if let Some(ref param) = qual.param
                    && let Ok(guard) = param.eval_value.lock()
                    && let Some(value) = guard.clone()
                {
                    qual.value = value;
                }
            }
